...
A trial-class booking system for classes with a capacity of 4: view classes, book a seat for a
child, complete a mock payment, see the confirmed roster. It is built to be correct when several
parents go for the same seat at the same time and when the same parent submits twice — not to be
feature-rich. There is no auth, no real payment provider and no UI polish. The whole design is one
idea: **a seat is a row, and a row is a mutex**, so overbooking is structurally impossible rather
than checked for.

Requirements this is judged against: [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md) (UC1–UC5,
NFR1–NFR23, and what is deliberately out of scope).

There is also an interactive walkthrough of the concurrency design in
[docs/slides.html](docs/slides.html) — open it in a browser, or run `pnpm slides`. It simulates
the last-seat race, compares `SKIP LOCKED` against `NOWAIT`, and lets you delete the row lock
and watch the invariant break.

## Contents

- [Quick start](#quick-start)
- [Verify the interesting behaviour in 2 minutes](#verify-the-interesting-behaviour-in-2-minutes)
...
Verified from a cold start on Postgres 14.19 / Bun 1.4.0 / pnpm 11.22.0 / k6 v2.1.0.

```sh
for db in roster roster_test roster_load; do createdb "$db"; done   # createdb takes ONE dbname
pnpm install
cp .env.example apps/api/.env     # API config
cp .env.example apps/web/.env     # Vite only reads VITE_* from here
pnpm db:reset                     # schema.sql + seed.sql into $DATABASE_URL
pnpm dev                          # API on $PORT, web on http://localhost:5173
```

Two things to check in the copied `.env` files before `pnpm db:reset`:

| If | Then |
|---|---|
| your Postgres superuser is not `postgres` (Homebrew creates a role named after your OS user) | edit the three DSNs in `apps/api/.env` |
| port 3000 is taken | set `PORT=3001` in `apps/api/.env` **and** `VITE_API_BASE_URL=http://localhost:3001` in `apps/web/.env` — they must agree |

`.env.example` is the complete list of variables and marks which app each belongs to. Both `.env`
files are gitignored.

```sh
pnpm test        # 19 integration tests over real HTTP against roster_test
pnpm typecheck   # all three packages
pnpm build       # web production build
./tests/load/run.sh booking-last-seat   # k6, resets roster_load, runs the API on :3999
```

---

## Verify the interesting behaviour in 2 minutes
...
Against [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md). Test names are as they appear in
...
| **NFR15** walkable in 5–8 min | [docs/slides.html](docs/slides.html); critical sections carry comments | met |
...
| **NFR19** CORS | `@elysiajs/cors` pinned to `CORS_ORIGIN` in `src/app.ts`; unset means "reflect any origin", acceptable locally only — [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) covers the deployed handshake | met |
| **NFR20** Docker container | `apps/api/Dockerfile` (repo-root build context, pnpm workspace) + `docker-compose.yml` | **partial — UNTESTED.** No Docker daemon was available on the build machine. Exact verification commands are in DEPLOYMENT.md. |
...
| **NFR23** README covers local / deploy / k6 | this file + [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) + [tests/load/README.md](tests/load/README.md) | met |
...
| Area | Time |
|---|---|
| Requirements, API contract (`packages/types`), schema and the three invariants — written by hand before any code | 30 min |
| API: `createBooking` / `completePayment`, routes, error mapping, env/logging | 55 min |
| Integration tests + the mutation testing pass that found the false passes | 50 min |
| k6 scripts and thresholds | 30 min |
| Frontend (three pages, TanStack Router + Query, Tailwind) | 25 min |
| Docker, deployment notes, README, video script | 20 min |
| **Total** | **~3.5 h** |
...
- **No authentication.** The client sends a `studentId` and is trusted. Roles ("parent", "admin")
  are conceptual; the roster page is not protected.
- **Mock payments settle synchronously** inside the request, driven by a `mockSuccess` boolean.
  There is no provider, no webhook and no asynchronous state.
- **One currency, one price** (2900 minor units). No tax, no discounts, no currency field.
- **No refunds, and none are needed:** a parent who loses the race is never charged. The lost-seat
  `payment_attempt` is recorded with `amount_cents = 0` and `failure_code = 'seat_lost'`, because it
  is an audit record of a race lost, not of money moved.
- **Capacity is fixed at 4** by the seed. `capacity` is a column and the code reads it, but there is
  no admin API to create classes or change capacity, so I1 cannot be violated at runtime.
- **All timestamps are `timestamptz`**, serialised as ISO 8601 UTC. The browser formats them with
  `toLocaleString()`; there is no timezone selection or presentation logic anywhere.
- **One API process** in development. The design does not depend on that — the invariants are in the
  database — but the connection pool sizing assumes it.

---

## What I deliberately cut

Per [REQUIREMENTS.md §3.8](docs/REQUIREMENTS.md), these are cuts, not omissions:

| Cut | Reason |
|---|---|
| Authentication / authorization | orthogonal to the concurrency problem being demonstrated; would have eaten the timebox in middleware. |
| Real payment gateway | the interesting failure ("I was charged for a seat I did not get") is reproducible with a boolean; a provider adds webhooks and async state without adding correctness. |
| Email / SMS notifications | no behaviour under test depends on them. |
| UI polish | the frontend exists to demonstrate the flow and to show that the UI's checks are cosmetic. Three pages, no design system. |
| Background sweeper for lapsed holds | lazy expiry in the claim predicate gets the same seat-reuse behaviour with no scheduler, no leader election and nothing to fail silently at 3am. The cost — stale `PENDING_PAYMENT` rows — is named above and monitored below. |
| Metrics / tracing | one-line JSON logs satisfy NFR14; a metrics stack is a deployment topic, not a correctness one. |
| A `capacity`-enforcing trigger for I1 | I1 is currently guaranteed by "seats are only ever created by `generate_series` and there is no admin API". A real structural guarantee needs a trigger or a capacity-bounded FK. Out of timebox for a scenario only reachable by ad-hoc DBA SQL. |
| Verified container build | the Docker daemon was not running on the build machine. Rather than claim it works, DEPLOYMENT.md says it is untested and gives the commands to check. |
...
1. **A sweeper for lapsed holds** — release the seat *and* mark the booking `CANCELLED`, so stale
   `PENDING_PAYMENT` rows stop accumulating and the parent gets told rather than discovering it on
   their next attempt.
2. **Idempotency keys on `POST /api/payments/complete`** — the status-transition guard is the
   minimum (NFR13); a client-supplied key would make retries safe across process restarts and let
   the same response be replayed byte-for-byte.
3. **A real provider with webhooks** — which means an async `PENDING` payment state, a seat hold
   that outlives the request, and reconciliation. The seat model already supports it: `locked` with
   a `pending_until` *is* the hold.
4. **Auth** — parent sessions, so `studentId` is derived from the session rather than trusted, and
   the roster view is admin-only.
5. **A waitlist for the losing racers** — the 19 parents who got `class_full` are exactly the demand
   signal you want to capture, and the seat rows make "notify when seat N frees" straightforward.
6. **Structured request-id logging** — a request id threaded through `log()` so a booking and its
   payment can be correlated across processes.
7. **CI running both suites** against a Postgres service container, plus the mutation checks as a
   scheduled job — a test that has never been shown to fail is not evidence.
...
roster/
├─ apps/
│  ├─ api/                        Elysia on Bun + postgres.js
│  │  ├─ db/schema.sql            the invariants live here
│  │  ├─ db/seed.sql              demo data: a free class, a 3/4 class, a full class
│  │  ├─ scripts/reset-db.ts      pnpm db:reset
│  │  ├─ src/services/booking.ts  createBooking + completePayment — the whole algorithm
│  │  ├─ src/routes/              students, classes, bookings, payments
│  │  ├─ src/{app,db,env,errors,log}.ts
│  │  ├─ tests/booking.test.ts    19 integration tests over real HTTP
│  │  └─ Dockerfile               build context is the REPO ROOT
│  └─ web/                        React 19 + TanStack Router/Query + Tailwind 4
│     └─ src/routes/              classes, book, roster
├─ packages/types/                the API contract, type-only, shared by both apps
├─ tests/load/                    three k6 scripts + run.sh
├─ docs/
│  ├─ REQUIREMENTS.md             use cases and non-functional requirements
│  ├─ DEPLOYMENT.md               Dokploy + Vercel
│  └─ slides.html                 interactive walkthrough of the concurrency design
├─ docker-compose.yml
└─ AI_USAGE.md
```
